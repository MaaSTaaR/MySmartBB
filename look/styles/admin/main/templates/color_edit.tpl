<br />

<div class="address_bar">لوحة التحكم &raquo; <a href="admin.php?page=toolbox&amp;colors=1&amp;control=1&amp;main=1">الالوان</a> &raquo; تحرير : {$Inf['name']}</div>

<br />

<form action="admin.php?page=toolbox&amp;colors=1&amp;edit=1&amp;start=1&amp;id={$Inf['id']}"  name="myform" method="post">
<table cellpadding="3" cellspacing="1" width="60%" class="t_style_b" border="1" align="center">
<tr valign="top" align="center">
	<td class="main1" colspan="2">تحرير لون</td>
</tr>
<tr valign="top">
		<td class="row1">رمز اللون</td>
		<td class="row1">
<input type="text" name="name" id="input_name" value="{$Inf['name']}" size="30" />&nbsp;
</td>
</tr>
</table><br />

<div align="center"><tr>
	<td class="submit-buttons" colspan="2" align="center">
	<input class="submit" type="submit" value="موافق" name="submit" accesskey="s" /></td>
</tr>
</table><br />
</form>
